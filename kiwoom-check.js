...
async function main() {
  ...
  const overseasAuth = await overseas.authenticate();
  const usBalance = await overseas.getUsBalance();
  const usCash = await overseas.getUsCash();
  console.log(`해외 모의투자 인증 성공 (토큰 만료: ${overseasAuth.expiresDt})`);
