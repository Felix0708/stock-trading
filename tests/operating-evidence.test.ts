...
(async () => {
  ...
  const pending = { ...order, status: "ACCEPTED", remainingQuantity: 51, reconciliationRequired: true };
  const cancelled = { ...row, filledQuantity: 0, remainingQuantity: 0, fillPrice: 0 };
  const cancellation = reconciliationPlan([pending], [cancelled], "mock").updates[0];
  assert.equal(cancellation.status, "CANCELLED");
  assert.equal(cancellation.filledQuantity, 0);
  assert.equal(cancellation.reconciliationRequired, false);
  assert.equal(reconciliationPlan([pending], [{ ...cancelled, remainingQuantity: 51 }], "mock").updates.length, 0);
