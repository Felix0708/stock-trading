...
const shadowServer = new TradeController({ initialMode: "SHADOW", accountNeutral: true });
assert.equal(shadowServer.evaluate(neutralPeg).verdict, "SHADOW_ENTRY");
assert.equal(shadowServer.evaluate(neutralDailyMixed).verdict, "SHADOW_ENTRY");
