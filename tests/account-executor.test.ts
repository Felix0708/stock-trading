...
assert.equal(shouldRetryMarketTransition(marketCloseRecord, new Error("[2000](RC4058:모의투자 장종료)"), afterOpen), false);
assert.equal(shouldRetryMarketTransition(marketCloseRecord, new Error("한투 모의 API 실패 [VTTT1001U]: 모의투자 장종료 입니다."), afterOpen), false);
assert.equal(shouldRetryMarketTransition(marketCloseRecord, new Error("장종료"), afterOpen, { id: "KIS", environment: "live" }), true);
assert.equal(shouldRetryMarketTransition(marketCloseRecord, new Error("장종료"), new Date("2026-09-01T13:30:05Z"), { id: "KIS", environment: "mock" }), true);
assert.equal(shouldRetryMarketTransition(marketCloseRecord, new Error("socket timeout"), afterOpen), false);
assert.equal(shouldRetryMarketTransition(marketCloseRecord, new Error("장종료"), new Date("2026-09-02T02:00:00.000Z")), false);
const transition = store.putDeferred("KIS", { ...marketCloseRecord, requestId: "request-transition" }, 600_000, { now: 1_000 });
let transitionState = store.markMarketTransitionFailure(transition.key, "2026-09-01:AFTER", new Error("장종료"), 1_000);
assert.equal(transitionState.orderRetryAttempts, 1);
assert.equal(transitionState.nextAttemptAt, 31_000);
assert.equal(deferredOrderAttemptDue(transitionState, "2026-09-01:AFTER", 30_999), false);
assert.equal(deferredOrderAttemptDue(transitionState, "2026-09-01:AFTER", 31_000), true);
transitionState = store.markMarketTransitionFailure(transition.key, "2026-09-01:AFTER", new Error("장종료"), 31_000);
assert.equal(transitionState.nextAttemptAt, 151_000);
transitionState = store.markMarketTransitionFailure(transition.key, "2026-09-01:AFTER", new Error("장종료"), 151_000);
assert.equal(transitionState.nextAttemptAt, 451_000);
