...
const { normalizeSignal } = require("../src/signals/signal-normalizer");
...
assert.equal(normalizeSignal({ action: "BUY", type: "🪜 강한 눌림목" }).signalCode, "ADD_STRONG_PULLBACK");
assert.equal(normalizeSignal({ action: "SELL", type: "🛑 급락 손절 -10%" }).signalCode, "EXIT_CRASH");
assert.equal(normalizeSignal({ action: "CHECK", type: "변동성 수축" }).signalCode, "VOLATILITY_CONTRACTION");
assert.equal(normalizeSignal({ action: "CHECK", type: "채널 이탈 홀딩" }).signalCode, "CHANNEL_EXIT_HOLD");
assert.equal(normalizeSignal({ action: "CHECK", type: "박스권 신규진입 차단" }).signalCode, "RANGE_ENTRY_BLOCK");
assert.equal(normalizeSignal({ action: "CHECK", type: "휩쏘" }).signalCode, "WHIPSAW_INFO");

const documentedSignals = [
  ["BUY", "💰 정석 진입"], ["BUY", "🚀 돌파 진입"], ["BUY", "⚡ 공격 진입"],
  ["BUY", "🔼 피라미딩 추매"], ["BUY", "🪜 강한 눌림목"], ["BUY", "📈 모멘텀 BUY"], ["BUY", "PEG Pullback"],
  ...
  ["CHECK", "변동성 수축"], ["CHECK", "채널 이탈 홀딩"], ["CHECK", "박스권 신규진입 차단"], ["CHECK", "휩쏘"],
