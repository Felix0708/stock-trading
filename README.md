...
`전체신호`에는 유효한 TradingView 신호를 모두 기록하고, 같은 신호를 성격에 따라 `관찰신호`, `진입신호`, `청산신호` 중 하나에도 보냅니다. `매매신호`에는 위험 게이트 통과나 승인 대기가 아니라 **키움이 실제 주문을 접수한 신호만** 추가로 보냅니다. 기존 `#매매신호`는 삭제하지 않고 `#매매신호-이전기록`으로 보존합니다.
...
- `매매 상태 보여줘`, `최근 주문 보여줘`, `매매 명령어 보여줘`는 조회 전용 자연어 명령입니다. 매매 모드·중지·재개처럼 상태를 바꾸는 명령은 안전을 위해 `!trade` 형식을 유지합니다.
...
- `#내-포트폴리오`는 연결된 키움 국내·미국 계좌의 현재 보유 종목을 10분마다, 그리고 체결 직후 같은 상태판 메시지에 반영합니다. 실제 채널명은 `.env`의 `MY_PORTFOLIO_CHANNEL`로 정합니다. 현재 `.env`가 모의투자이면 모의계좌만 표시됩니다.
...
아래 정책은 `Lazy Alpha Indicator`의 공식 매매 원칙이 아니라 이 프로젝트가 Webhook 신호 위에 별도로 적용하는 로컬 위험 관리 규칙입니다. 기본값은 `SHADOW`이며, 동시에 보유할 수 있는 종목은 최대 5개입니다. `!trade paper`로 바꾸면 유효한 일반 TradingView 진입·부분청산·전량청산 신호를 키움 모의계좌로 보냅니다. 실계좌 주문은 계속 차단합니다.
