...
| `npm run portfolio:refresh` | 무니 자료와 생존 투자자의 최신 공개 포트폴리오·언급 종목 즉시 갱신 |
...
- `#주요인사-포트폴리오`는 드러켄밀러·버핏·캐시 우드·마이클 버리·조지 소로스·레오폴드 아셴브레너의 최신 공개 자료와 무니 자료를 하루마다 확인해 하나의 상태판 메시지만 수정합니다. 각 인물은 최신 공시의 상위 7개 보유 항목을 비중순으로 표시합니다.
- `#기관-포트폴리오`는 시타델·블랙록·뱅가드·스테이트스트리트·피델리티·캐피털그룹·JP모건·골드만삭스·모건스탠리·르네상스 테크놀로지스의 최신 SEC 13F 상위 4개 항목을 비중순으로 정리해 별도의 상태판 메시지만 수정합니다.
- 사용자별 포트폴리오 채널은 연결된 키움·한투 모의계좌의 현재 보유 종목을 같은 상태판 메시지에 반영합니다. 실제 채널명은 `.env.account`의 `ACCOUNT_PORTFOLIO_CHANNEL`로 정합니다.
- 무니는 최근 31일의 무니인사이트 Markdown·PDF에서 실제 매입·보유, 관심 종목, 단순 언급을 구분합니다. 이 문맥은 다섯 봇이 다음 답변부터 함께 읽습니다.
...
- `!trade live`는 지원하지 않습니다. 모의투자 검증과 체결 추적을 완료한 뒤 별도로 추가합니다.
...
- 국내·미국 BUY 신호: 사용자별 키움·한투 모의계좌의 현금·보유·비중을 기준으로 수량·투자금·예상 손실 계산 후 승인 대기
...
피라미딩의 1차 50%·2차 25%처럼 최초 수량을 기준으로 하는 단계별 수량은 아직 연결하지 않았습니다. 키움 잔고만으로 최초 진입분과 이후 추가분을 안전하게 구분할 수 없기 때문입니다. 실계좌 주문도 구현하지 않았습니다.
