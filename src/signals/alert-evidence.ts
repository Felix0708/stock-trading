...
function applyAlertSnapshot(evidence, items, snapshot, now = new Date()) {
  ...
  if (!Number.isFinite(age) || age < 0) throw Error("알람 확인 자료의 확인 시각이 잘못되었습니다. 현재 화면을 다시 확인하세요.");
  if (!Array.isArray(snapshot?.rows) || snapshot.rows.length > 2000
    || snapshot.source !== "TradingView alert manager visible UI") throw Error("알람 확인 자료의 형식·출처가 잘못되었습니다.");
  // Old observations are not corrupt data and must not renew active status.
  // The registry reports these as '확인 필요' using their original verifiedAt.
  if (age > 7 * 86400_000) return 0;
  let applied = 0;
...
function alertEvidenceSummary(items, evidence = {}, now = new Date()) {
  const rows = items.flatMap(item => ["240", "1D"].map(timeframe => {
    ...
    return { key, ...proof, status: verified ? "활성 확인" : proof.expiresAt && Date.parse(proof.expiresAt) <= now.getTime() ? "만료" : proof.active === false ? "중단 확인·재확인 필요" : "확인 필요" };
  }));
